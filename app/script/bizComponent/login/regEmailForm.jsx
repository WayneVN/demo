...
var MergerModal = require('../../model/userModel').default;
var LoginActions = require('../../action/loginAction');
var ReloadAction = require('../../action/reloadAction');
...
var RegEmailForm = React.createClass({
  ...
  onSend:function(e){
    ...
    if (check) {
      ...
      new MergerModal().signup({uname:this.state.email,pwd:this.state.pwd,repwd:this.state.repwd},function(data){
        var result ;
        if (data.status) {
            new MergerModal().sendRegisterLog();
            new MergerModal().login({uname:self.state.email,pwd:self.state.pwd},function(data){
            ReloadAction.reload();
                ...
                <div className="form-footer">
                  ...
                  <ul className="form-sso">
                    {/*
                      ...
                      */}
                    <li>
                      <ul className="form-btn-qq" onClick={e =>{DialogAction.open(Dialog.RegTelForm)}} style={{marginLeft:10,marginTop:50,}}>
