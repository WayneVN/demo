...
const Table = React.createClass({
  ...
  getData: function () {
    SummerModel.getJobList(this.props.data.uid, this.props.courseId, result => {
      let {
        status,
        data = [],
        message = ""
      ...
      if (status) {
        this.setState({
          list: data.map((item,k) => {
            // 转换一下数据
            item.job_status = config.jobStatus[+item.job_status];
            item.job_score_level = config.jobScoreLevel[+item.job_score_level];
    ...
    if (isdow) {
      SummerModel.getmyAnswer(uid, jobid, courseId, result => {
        this.setState({
          ...
          timebeg: timebeg,
          timeend: timeend,
          dowUrl: `/camp/jobfiles/${this.props.courseId}${result.data.my_answer}`
        });
      });
    }
    else {
  ...
  postFile: function () {
    ...
    data.append('course_id',this.props.courseId);
