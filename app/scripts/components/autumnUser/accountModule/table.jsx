...
const Table = React.createClass({
  ...
  checkModal: function () {
    logger.log({
      target: 'autumn2016_account_open_upload'
    });
    this.setState({
  ...
  checkEvaluate: function () {
    logger.log({
      target: 'autumn2016_account_open_checkEvaluate'
    });
    this.setState({
    ...
    if (isdow) {
      SpringModel.getmyAnswer(uid, jobid, result => {
        this.setState({
          ...
          timebeg: timebeg,
          timeend: timeend,
          dowUrl: `/autumn/basic/jobfiles${result.data.my_answer}`
        });
      });
    }
    else {
  ...
  postFile: function () {
    ...
    let url =  `/autumn/basic/students/jobUpload`;
