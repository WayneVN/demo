...
const Answer = React.createClass({
  ...
  getAnswer: function () {
    ...
    SpringModel.jobAnswers(uid, this.props.params.courseId, result => {
      const {
        status,
        data = []
      } = result;
  ...
  unlock: function(item) {
    ...
    DialogAction.open(Dialog.Unlock, obj);
  },
                ...
                return (
                  <tr key={k}  >
                    <td style={{position: "relative"}}>
                      {k+1}
                      {!item.file?<div className="lock-row" onClick={e => {this.unlock(item)}} />: <noscript /> }
                      {!item.file?<img  src="images/lock_icon.png" className="lock-img" onClick={e => {this.unlock(item)}} />: <noscript/>}
                    ...
                    <td>
                      <span onClick= {event => {this.showDetail(item.id)}}
                            className={`jd-btn ${item.file?'jd-btn-orange':'jd-btn-gray'}`}
                      >
                        查看
                      </span>
                  &nbsp;
                  <a href={
                    item.help_file?
                           `/camp/jobfiles/${this.props.params.courseId}${item.help_file}`:
                           'javascript:;'
                          }
                     target="_blank"
  ...
  showDetail: function(id) {
    this.filterData(this.state.tableList, id, (showList) => {
      this.setState({
        ...
        imgUri:`camp/jobfiles/${this.props.params.courseId}/answers/${ showList[0] }`
      });
    });
  },
  ...
  renderUrl: function(uri) {
    this.setState({
      imgUri: `camp/jobfiles/${this.props.params.courseId}/answers/${ uri }`
    });
  },
