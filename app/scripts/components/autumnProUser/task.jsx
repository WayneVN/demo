...
const Task = React.createClass({
  ...
  getQuestions: function () {
    ...
    SpringModel.jobQuestions(user_id, result => {
      const {
        status,
        data = []
      } = result;
                    ...
                    <td>{
                      ...
                      <td>{item.job_title}</td>
                        <td>
                            ...
                            &nbsp;
                            <a href={item.help_file?`autumn/advanced/jobfiles${item.help_file}`:'javascript:;'}
                               target="_blank"
  ...
  showDetail: function(id) {
    this.filterData(this.state.tableList, id, (showList) => {
      this.setState({
        ...
        imgUri:`autumn/advanced/jobfiles/jobs/${ showList[0] }`
      });
    });
  },
  ...
  renderUrl: function(uri) {
    this.setState({
      imgUri: `autumn/advanced/jobfiles/jobs/${ uri }`
    });
  },
    ...
    return (
      <div>
        ...
        <div className="page-header">
          <h2>
            <a href="javascript:;" onClick={event => {this.setState({isBack: false})}}>秋训营作业列表</a>
            <If when={isBack}>
              <small>&nbsp;>&nbsp;{title}&nbsp;</small>
            </If>
            <If when={isBack}>
