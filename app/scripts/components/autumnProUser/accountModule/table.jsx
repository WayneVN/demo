...
const Table = React.createClass({
  ...
  checkModal: function () {
    logger.log({
      target: 'autumnPro2016_account_open_upload'
    });
    this.setState({
  ...
  checkEvaluate: function () {
    logger.log({
      target: 'autumnPro2016_account_open_checkEvaluate'
    });
    this.setState({
  ...
  openUpload: function(jobid, isdow, timebeg, timeend) {
    ...
    if (isdow) {
      SpringModel.getmyAnswer(uid, jobid, result => {
        this.setState({
          show: !this.state.show,
          jobid: jobid,
          isdow: isdow,
          timebeg: timebeg,
          timeend: timeend,
          dowUrl: `/autumn/advanced/jobfiles${result.data.my_answer}`
        });
      });
    }
    else {
  ...
  postFile: function () {
    ...
    let url =  `/autumn/advanced/students/jobUpload`;
