...
const TaskTable = React.createClass({
  ...
  componentWillReceiveProps: function(nextProps) {
    this.setState({
      ...
      this.getAssitData(nextProps.data.uid, this.props.params.id );
    });
  ...
  getData: function() {
    ...
    this.getAssitData(this.props.data.uid, job_id);
  },

  // 批量下载用户的作业
  getDownloadJobs: function() {
    let {
      state: {
        job_id,
        level,
    ...
    SpringModel.getDownloadJobs(params, result => {
      if (result.status) {
        window.open(`camp/jobfiles/${this.props.selectVal}/${ result.data.download_file }`);
      }
      else {
  ...
  // 批量下载用户的批改结果信息
  getDownloadCorrectInfo: function() {
    let {
      state: {
        job_id,
        level,
    ...
    SpringModel.getDownloadCorrectInfo(params, result => {
      if (result.status) {
        window.open(`camp/jobfiles/${this.props.selectVal}/${ result.data.download_file }`);
      }
      else {
  ...
  getAssitData: function(uid, job_id) {
    SpringModel.assitStudentsJob(this.props.data.uid, job_id, this.props.selectVal, result => {
      let {
        status,
        data
      } = result;
  ...
  getAdminData: function(uid, job_id) {
    SpringModel.adminStudentsJob(uid, job_id, this.props.selectVal, result => {
      let {
        status,
        data
      } = result;
  ...
  subCommit: function () {
    ...
    let course_id = this.props.selectVal;
        ...
        <div className="page-header">
          <h2></h2>
            <a href="javascript:;"
               ...
               onClick={event => { this.getDownloadJobs() }}
              >
              作业批量下载
            </a>
            &nbsp;
            <a href="javascript:;"
               className="jd-btn jd-btn-orange"
               onClick={event => { this.getDownloadCorrectInfo() }}
            >
              批改结果下载
            </a>
        </div>
                          ...
                          <td>{SpringMap.jobScoreLevel[item.score_level]}</td>
                            ...
                            <td>
                                <If when={item.feedback_file_link}>
                                    <a href={`/camp/jobfiles/${this.props.selectVal}${ item.feedback_file_link }`}
                                         target="_blank">
                                        点击下载
                                      </a>
                                </If>
