...
const Table = React.createClass({
  ...
  componentDidMount: function() {
    this.getData(this.props.userInfo.user_id);
  },

  componentWillReceiveProps: function(nextProps) {
    this.getData(nextProps.userInfo.user_id);
  },

  getData: function(user_id) {
    SpringModel.adminUserInfo(user_id, result => {
      let {
        status,
        data
      } = result;
          ...
          <table className="table table-condensed" style={{width:'810'}}>
            ...
            <tbody>
              ...
              {list.map( (item,k) => {
                ...
                <td>
                  <a href={`#/autumnproadmin/single/${ item.user_id }`}
