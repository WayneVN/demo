...
(function ($) {
    ...
    var DEFAULT_OPTS = {
        ...
        isShowDateRange:true, // 是否展示 昨天/最近7天 等;
        selectedRange: null,
        showAlways: false,
        hideOnClick: true,
        ...
        template: [
            ...
            '<span class="daterangepicker-time-text"></span>',
    ...
    function DateRangePicker(element, options) {
        ...
        if (!options.isShowDateRange) {
          options.template=options.templateNoDate
        }

        // hanrui扩展zIndex，系统默认1000，但是$modal是1050，所以需要用户指定
        ...
        // hanrui 放到body中，而不是放到el后，不然位置有问题
    ...
    DateRangePicker.prototype.update = function (option) {
        if (option.startTime) {
            this.daterangepicker.find('[data-type="startTime"]').datetimepicker({
                selectedDate: option.startTime
            });
        }
        
        if (option.endTime) {
            this.daterangepicker.find('[data-type="endTime"]').datetimepicker({
                selectedDate: option.endTime
            });
        }
